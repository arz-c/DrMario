...
# - initially 45, corresponding roughly to 1 second in 60 FPS
max_frames:
  .word 0
...
main:
  ...
  li $t0, 45           # $t0 = 45
  sw $t0, max_frames   # initialize max_frames to 45

  li $a0, 4            # $a0 (number of viruses) = 4
  jal create_viruses   # initialize 4 viurses with random positions/colours
...
game_loop:
  # 0. If no viruses remain on the field, reset the game
  lw $t0, viruses    # $t0 = viruses
  beq $t0, 0, main   # if viruses == 0, jump to main
  ...
  key_pressed:
    ...
    beq $t1, 0x70, pause          # check if 'p' was pressed
    beq $t1, 0x71, quit           # check if 'q' was pressed
...
create_pill:
  la $s1, pill        # $s1 = address of pill
  addi $sp, $sp, -4   # allocate 1 word in stack
  sw $ra, 0($sp)      # push return address to stack
  
  li $s2, 300
  sw $s2, 0($s1)      # pill[0] = 300 = (11, 2)
  jal random_colour   # call helper function
  sw $v0, 4($s1)      # pill[1] = returned colour

  li $s2, 428
  sw $s2, 8($s1)      # pill[2] = 428 = (11, 3)
  jal random_colour   # call helper function
  sw $v0, 12($s1)     # pill[3] = returned colour
...
create_viruses:
  ...
  create_viruses_start:
    # TODO: two viruses can currently be initialized to the same location
    ...
    # generate a random integer (y coordinate) between 5 and 28, inclusive
    li $v0, 42   # syscall config
    li $a0, 0    # RNG id
    li $a1, 24   # maximum (exclusive)
    syscall      # return value is $a0
    addi $a0, $a0, 5   # shift return value by desired minimum
    ...
    # (this can technically fall into very long loops, but we will ignore the highly unlikely possibility)
...
check_pill_landed:
  ...
  pill_landed:
    ...
    ble $t4, 560, main   # if position of pixel 1 <= (12, 4), then game is over
...
pause:
  ...
  addi $t3, $t0, 360   # current pixel = address of (26, 2)
...
eliminate_field:
  ...
  drop_field:
    ...
    drop_field_to_repeat:
      ...
      # sleep
      li $v0, 32    # set syscall operation to sleep
      li $a0, 150   # sleep duration is 150 ms (~6.7 FPS)
      syscall
...
eliminate_vertical:
  ...
  li $t7, 0            # $t7 = boolean for whether current same-colour line includes a virus
  ...
  eliminate_vertical_start:
    ...
    eliminate_vertical_not_black:
      ...
      # if one pixel is a virus of matching colour to the other pixel:
      beq $t6, 0x333333, eliminate_vertical_match_v    # check if difference is 0x333333
      beq $t6, -0x333333, eliminate_vertical_match_v   # check if difference is -0x333333
      # else, if the two pixels are the (exact) same colour:
      beq $t6, 0, eliminate_vertical_match   # check if difference is 0
      # else:
      j eliminate_vertical_mismatch   # otherwise, colours mismatch
      eliminate_vertical_match_v:
        li $t7, 1   # set boolean to 1 to indicate that a virus has been found
        # then continue to eliminate_vertical_match
      eliminate_vertical_match:
        # increment count by 1, then increment and continue
        addi $t4, $t4, 1   # add 1 to count
        j eliminate_vertical_increment   # increment and continue
      # if current colour and previous colour mismatch:
      eliminate_vertical_mismatch:
        # set previous colour to current colour
        add $t3, $t5, $zero   # previous colour $t3 = current colour $t5
        # if count >= 4:
        ...
        eliminate_vertical_ge4:
          ...
          # if the just-eliminated line contained a virus:
          beq $t7, 1, eliminate_vertical_virus_elimed
          j eliminate_vertical_increment   # else, increment and continue
          eliminate_vertical_virus_elimed:
            # decrement viruses by 1
            la $t7, viruses     # $t7 = address of viruses (temporarily using $t7 for a computation)
            lw $t6, viruses     # $t6 = viruses
            addi $t6, $t6, -1   # $t6 = viruses - 1
            sw $t6, 0($t7)      # viruses = $t6
            li $t7, 0           # reset boolean $t7 to 0
            beq $t6, 2, eliminate_vertical_enhance_gravity   # if viruses == 2
            j eliminate_vertical_increment   # otherwise, increment/continue
            eliminate_vertical_enhance_gravity:
              # enhance gravity
              lw $t6, max_frames     # $t1 = max_frames
              sra $t6, $t6, 1        # $t1 /= 2
              sw $t6, max_frames     # max_frames = $t1
              li $t6, 0              # $t1 = 0
              sw $t6, frames_delta   # frames_delta = $t1
              j eliminate_vertical_increment   # increment and continue
...
eliminate_horizontal:
  ...
  eliminate_horizontal_start:
    ...
    eliminate_horizontal_not_black:
      sub $t6, $t5, $t3   # $t6 = current colour - previous colour
      # if one pixel is a virus of matching colour to the other pixel:
      beq $t6, 0x333333, eliminate_horizontal_match_v    # check if difference is 0x333333
      beq $t6, -0x333333, eliminate_horizontal_match_v   # check if difference is -0x333333
      # else, if the two pixels are the (exact) same colour:
      beq $t6, 0, eliminate_horizontal_match   # check if difference is 0
      # else:
      j eliminate_horizontal_mismatch   # otherwise, colours mismatch
      eliminate_horizontal_match_v:
        li $t7, 1   # set boolean to 1 to indicate that a virus has been found
        # then continue to eliminate_horizontal_match
      ...
      eliminate_horizontal_mismatch:
        ...
        eliminate_horizontal_ge4:
          ...
          li $t2, 1   # set boolean to 1 to indicate that a line was eliminated
          # if the just-eliminated line contained a virus:
          beq $t7, 1, eliminate_horizontal_virus_elimed
          j eliminate_horizontal_increment   # else, increment and continue
          eliminate_horizontal_virus_elimed:
            # decrement viruses by 1
            la $t7, viruses     # $t7 = address of viruses (temporarily using $t7 for a computation)
            lw $t6, viruses     # $t6 = viruses
            addi $t6, $t6, -1   # $t6 = viruses - 1
            sw $t6, 0($t7)      # viruses = $t6
            li $t7, 0           # reset boolean $t7 to 0
            beq $t6, 2, eliminate_horizontal_enhance_gravity   # if viruses == 2
            j eliminate_horizontal_increment   # otherwise, increment/continue
            eliminate_horizontal_enhance_gravity:
              # enhance gravity
              lw $t6, max_frames     # $t1 = max_frames
              sra $t6, $t6, 1        # $t1 /= 2
              sw $t6, max_frames     # max_frames = $t1
